  ...
  return (
    <>
      ...
      <nav className="fixed top-0 left-0 w-full z-50 flex items-center justify-between bg-white/95 backdrop-blur-md shadow-md/5 px-4 lg:px-6 py-2 font-[Poppins]">
        {/* Logo */}
        <h2 className="relative text-4xl font-bold flex items-center">
            {/* A ke liye professional SVG logo */}
            <svg
              xmlns="public/img/a48e8f14-6a1c-48a3-b027-7d72308475f0.jpg"
              viewBox="0 0 54 55"
              className="w-10 h-10 text-blue-700"
              fill="currentColor"
            >
              <path d="M32 2 L12 62 H22 L32 32 L42 62 H52 L32 2 Z" />
            </svg>

            {/* Baaki letters A ke bilkul upar chipka hua */}
            <span
              className="absolute left-8 top-0 text-blue-800 text-2xl"
            ...
            {menuItems.map((item) => (
              <li key={item}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `pb-1 transition-all duration-300 ease-in-out ${isActive
      ...
      <div
        className={`fixed top-14 right-0 h-full w-3/4 sm:w-1/2 bg-white z-50 shadow-2xl/20 border-t border-gray-300 transform duration-500 ease-in-out lg:hidden ${menuOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        {/* Sidebar Top */}
        <div className="flex items-center justify-between px-4 py-4 bg-blue-50 border-b border-orange-200 rounded-bl-xl">
          <h2 className="relative text-4xl font-bold flex items-center">
            ...
            {/* Baaki letters A ke bilkul upar chipka hua */}
            <span
              className="absolute left-8 top-0 text-blue-900 text-2xl"
              style={{ fontFamily: "'Billabong', cursive" }}
            >
              nalytics
            </span>
          ...
          {menuItems.map((item) => (
            <li key={item}>
              <NavLink
                ...
                onClick={() => setMenuOpen(false)}
                className={({ isActive }) =>
                  `block py-3 text-lg transition-all rounded-lg ${isActive
